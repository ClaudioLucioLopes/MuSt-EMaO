...
def call_CTAEA(problem, n_points, n_obj, data_unit_simplex, sampling_p, n_gen, n_eval, termination=None, seed_p=1):
    ...
    res = minimize(problem, algorithm, termination)
    n_var = problem.n_var
    if (np.sum(res.pop.get('F') is None)) == 1:
        n_evals = res.algorithm.evaluator.n_eval
        n_gen = res.algorithm.n_gen
        return np.full([ 1, n_var ], np.nan), np.full([ 1, n_obj ], np.nan), \
            np.full([ 1, n_var ], np.nan), np.full([ 1, n_obj ], np.nan), np.full([ 1, n_obj ], np.nan), np.full(
...
def call_MOEAD(problem, n_points, n_obj, data_unit_simplex, sampling_p, n_gen, n_eval, termination=None, seed_p=1):
    if len(sampling_p) == 0:
        ...
        algo_m = MOEAD(data_unit_simplex, prob_neighbor_mating=0.7, sampling=sampling_p,
                       n_neighbors=int(0.3 * n_points), seed=seed_p)
    if termination == None:
        if n_eval != 0:
            termination = get_termination("n_eval", n_eval)
        else:
...
def Classify(PF, reference, ideal_point, nadir_point):
    ...
    return reference[ np.unique(niche_of_individuals), : ], ref_not_in_closest_reference, dist_matrix, final_closest, \
        closest_PF[ closest_non_dominated_index ]
...
def MuSt_EMaO(problem, n_points, n_obj, seed_p, total_neval_par, gamma, logprint, alg='NSGA-III'):
    ...
    if alg == 'NSGA-III':
        S1_all_x_var, S1_all_f_var, S1_ND_x_var, S1_ND_f_var, ideal_point_S1, nadir_point_S1, n_eval_1, n_gen1 = call_NSGAIII(
    ...
    if N1 < int(n_points):
        if alg == 'NSGA-III':
            ...
            S2_all_x_var, S2_all_f_var, S2_ND_x_var, S2_ND_f_var, ideal_point_S2, nadir_point_S2, n_eval_2, n_gen2 = call_CTAEA(
                problem, R_1_cbar.shape[ 0 ],
                n_obj, R_1_cbar, S1_all_x_var, 0, E2, None, seed_p)

        total_neval += n_eval_2
        total_ngen += n_gen2

        # Update the budget for the stage 3
        E3 = int(total_neval_par - total_neval)
